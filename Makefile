...
ACLOCAL = /home/usrc/tinyproxy-1.6.3/missing aclocal-1.4
AUTOCONF = autoconf
AUTOMAKE = /home/usrc/tinyproxy-1.6.3/missing automake-1.4
...
LN_S = ln -s
LTLIB = @LTLIB@
MAKEINFO = /home/usrc/tinyproxy-1.6.3/missing makeinfo
OBJDUMP = @OBJDUMP@
OBJEXT = o
PACKAGE = tinyproxy
RANLIB = ranlib
...
REDHAT =  	$(REDHAT_DIR)/tinyproxy-1.5.2-config-patch 	$(REDHAT_DIR)/tinyproxy-initd 	$(REDHAT_DIR)/tinyproxy.spec
...
config.h.in config.sub configure configure.ac install-sh ltmain.sh \
...
distdir: $(DISTFILES)
	...
	mkdir $(distdir)
	-chmod 777 $(distdir)
	$(mkinstalldirs) $(distdir)/$(REDHAT_DIR)
...
tinyproxy-configure-file:
	$(mkinstalldirs) $(DESTDIR)/usr/local/etc/tinyproxy
	$(INSTALL) -m 600 $(srcdir)/doc/tinyproxy.conf \
		$(DESTDIR)/usr/local/etc/tinyproxy/tinyproxy.conf-dist
	test -f $(DESTDIR)/usr/local/etc/tinyproxy/tinyproxy.conf \
		|| $(INSTALL) -m 600 $(srcdir)/doc/tinyproxy.conf \
			$(DESTDIR)/usr/local/etc/tinyproxy/tinyproxy.conf
	@echo ""
	@echo "A configuration file has been copied to:"
	@echo ""
	@echo "/usr/local/etc/tinyproxy/tinyproxy.conf-dist"
	@echo ""
	@echo "You will need to rename this file to:"
	@echo ""
	@echo "/usr/local/etc/tinyproxy/tinyproxy.conf"
	@echo ""
